...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        // check if we need to traverse left or right child.
        // if so, recursively call deleteElement() along proper subtree.
        if (value < node.value) {
            ...
            if (node.leftChild == null || node.rightChild == null) {
                // temp variable used for a non-null child if there is one.
                // if left is not null, then temp will be assigned to left child, otherwise
                // assigned to the right child.
                Node temp = (node.leftChild != null) ? node.leftChild : node.rightChild;
                // check if node has no sub-children. If so, set node to null.
                if (temp == null) {
                    temp = node;
                    node = null;
                    // node does have one child, so replace node with its child using temp variable
                } else {
                    node = temp;
                }
            // node has two children
            } else {
                // find in-order successor node (smallest node in right subtree)
        ...
        // recalculate nodes height
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
        // balance = height of left subtree - height of right subtree

        int balance = getBalanceFactor(node);

        // if left heavy and the left child's bf is >/= 0
        // There is a LL imbalance.
        ...
        // if right heavy and the right child's bf is </= 0
        // there is a RR imbalance.
